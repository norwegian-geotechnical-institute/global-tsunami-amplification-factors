...
class sio:
    ...
    def read(self,file,nds=False,unsrt=False):
        ...
        if type==ONEDIM:
            ...
            if type==GPHOV:
                self.pscr(file+" is a GPHOV-file")
                self.input=open(self.infile,'r').read()
                self._parse_gphov()
            elif type==MTV:
                self.pscr(file+" is a MTV-file")
                self.input=open(self.infile,'r').read()
                self._parse_mtv()
            elif type==XYZ:
                self.pscr(file+" is a XYZ-file")
                self.inarray=numpy.loadtxt(self.infile)
                self._parse_xyz(unsrt=unsrt)
            elif type==GRD:
                self.pscr(file+" is a grd-file (surfer)")
                self.input=open(self.infile,'r').read()
                self._parse_grd()
            elif type==MOST:
                self.pscr(file+" is a most grid-file (ascii)")
                self.input=open(self.infile,'r').read()
                self._parse_most()
            elif type==ESRI:
                self.pscr(file+" is a ESRI ASCII file")
                self._parse_esri(file)
            elif type==GCL:
                self.pscr(file+" is a GeoClaw type ESRI ASCII file")
    ...
    def _list2array(self,list):
        ...
        if self.no>0 and len(arr)/self.no!=self.nx*self.ny:
            print("in sct.io._list2array: error! Incorrect length of array,"+str(len(arr)),"vs.",str(self.nx*self.ny)+".")
        return arr
    ...
    def _string2array(self, string):
        ...
        #check number of entries:
        if self.no>0 and len(arr)/self.no!=self.nx*self.ny:
            print("in sct.io._string2array: error! Incorrect length of array,"+str(len(arr)),"vs.",str(self.nx*self.ny)+".")
        return arr
    ...
    def write(self,filename,x,y,z=[],type=None, NC=NC,txt="",degr=True,nds=False,ncfield="z",sortcol=0):
        ...
        if type=="mtv" or type=="MTV":
            ...
            self._write_xyz(sortcol=sortcol)
            ...
            if not degr:
                x="x"
                y="y"
            self._write_netcdf(ncfield,x,y)
    ...
    def _write_netcdf(self,field="Field",x="lon",y="lat"):
    ...
    def _gphov_index(self):
        ...
        if self.nds:
            no=self.nds
        index="%6d%6d%6d%12.6f%12.6f%12.6f%12.6f%6d"\
